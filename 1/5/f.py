...
# Администрации школы предоставили список из m различных моделей кондиционеров, которые можно закупить. Для каждой модели кондиционера известна его мощность и стоимость.
# Требуется написать программу, которая определит, за какую минимальную суммарную стоимость кондиционеров можно оснастить все классы школы.
...
n = int(input())
min_power = list(map(int, input().split()))
m = int(input())
ac = list()
for _ in range(m):
...
# отсортировать классы по возрастанию мощности кондиционера
min_power.sort()
# отсортировать кондиционеры по возрастанию цены и по мощности
ac.sort(key=itemgetter(1, 0))
total = 0
i = 0
