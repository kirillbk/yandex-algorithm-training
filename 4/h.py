...
# Одна из проблем расшифровки письменности Майя заключается в определении этого порядка. Рисуя значки некоторого слова, писатели Майя иногда выбирали позиции для значков, исходя скорее из эстетических взглядов, а не определенных правил.
...
# Поскольку они знают, что Вы приедете на IOI ’06, они просят Вас о помощи. Они дадут Вам g значков, составляющих слово W, и последовательность S всех значков в надписи, которую они изучают, в порядке их появления. Помогите им, подсчитав количество возможных появлений слова W.
...
# Модификация решения из разбора, использовал сортировку подсчётом

def update_counter(cntr_window, cntr_word, i, value):
	diff = 0
	if cntr_word[i] and cntr_window[i] == cntr_word[i]:
		diff = -1
	cntr_window[i] += value
	if cntr_word[i] and cntr_window[i] == cntr_word[i]:
		diff = 1
...
# два списка для подсчёта количества символов (используется ~половина от 127)
cntr_word = [0] * 127
cntr_window = [0] * 127
# подчёт количества символов в g первых символах w и str
for i in range(g):
	cntr_word[ord(w[i])] += 1
	cntr_window[ord(str[i])] += 1
# word_cntr_len - количество уникальных символов в w
# matches - количество символов, встречающихся в cntr_word и cntr_window одинаковое число раз 
word_cntr_len = 0
matches = 0
for i in range(65, 123):
	if cntr_word[i]:
		word_cntr_len += 1
		if cntr_word[i] == cntr_window[i]:
			matches += 1
result = 0
if matches == word_cntr_len:
	result += 1
for i in range(g, s):
	# на каждой итерации цикла добавляем/удаляем из cntr_window символ из str, обновляем количество совпадений(matches) с cntr_word
	matches += update_counter(cntr_window, cntr_word, ord(str[i - g]), -1)
	matches += update_counter(cntr_window, cntr_word, ord(str[i]), 1)
	if matches == word_cntr_len:
		result += 1
